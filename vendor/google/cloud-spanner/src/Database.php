...
class Database
{
    ...
    public function execute($sql, array $options = [])
    {
        $start = hrtime(true);
        $session = null;
        Tracer::inSpan(['name' => 'db_execute_processing'], function () use(&$options, &$session) {
            unset($options['requestOptions']['transactionTag']);
            $session = Tracer::inSpan(['name' => 'acquire_session'], function () use(&$options){

                return $this->pluck('session', $options, false)
                ?: $this->selectSession(
                    SessionPoolInterface::CONTEXT_READ,
                    $this->pluck('sessionOptions', $options, false) ?: []
                );
        ...
        try {
            $resp = Tracer::inSpan(['name' => 'operation_execute'], [$this->operation, 'execute'], [$session, $sql, $options]);
            BaseStub::$logs[] = ['db_execute', '', hrtime(true) - $start];
            return $resp;
        } finally {
            $session->setExpiration();
        }
