...
class Grpc implements ConnectionInterface
{
    ...
    {
        $databaseName = '';
        Tracer::inSpan(['name' => 'connection_class_processing'], function () use(&$args, &$databaseName){
            $start = hrtime(true);
            $a = hrtime(true);
            ...
            $c = hrtime(true) - $c;
            $s1 = hrtime(true) - $start;
            BaseStub::$logs[] = ['Grpc_before_call', 'Time taken by connection class w/o call.', $s1];
            BaseStub::$logs[] = ['Grpc_executeStreamingSql', 'Time taken by the connection class to return the response.', hrtime(true) - $start];
        });
        $resp = Tracer::inSpan(['name' => 'GrpcTrait->send'], [$this, 'send'], [
        ...
        // BaseStub::$logs[] = ['Grpc_before_call_A1', 'Time taken by connection class w/o call(A).', $a1];
        // BaseStub::$logs[] = ['Grpc_before_call_A2A', 'Time taken by connection class w/o call(A).', $a2a];
        // BaseStub::$logs[] = ['Grpc_before_call_A2B', 'Time taken by connection class w/o call(A).', $a2b];
        // BaseStub::$logs[] = ['Grpc_before_call_A2', 'Time taken by connection class w/o call(A).', $a2];
        // BaseStub::$logs[] = ['Grpc_before_call_A3', 'Time taken by connection class w/o call(A).', $a3];
        // BaseStub::$logs[] = ['Grpc_before_call_A', 'Time taken by connection class w/o call(A).', $a];
        // BaseStub::$logs[] = ['Grpc_before_call_B', 'Time taken by connection class w/o call(B).', $b];
        // BaseStub::$logs[] = ['Grpc_before_call_C', 'Time taken by connection class w/o call(C).', $c];
    ...
    private function createTransactionSelector(array &$args)
    {
        ...
        if (isset($args['transaction'])) {
            ...
            $selector->mergeFromJsonString(json_encode($transaction));
            // BaseStub::$logs[] = ['Grpc_decodeMessage', '.', hrtime(true) - $start];
            // $start = hrtime(true);
            // $selector2 = new TransactionSelector;
            // $selector2->mergeFromJsonString(json_encode($transaction));
            // BaseStub::$logs[] = ['Grpc_alt_decodeMessage', '.', hrtime(true) - $start];
