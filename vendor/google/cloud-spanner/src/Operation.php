...
class Operation
{
    ...
    public function execute(Session $session, $sql, array $options = [])
    {
        ...
        $call = function ($resumeToken = null, $transaction = null) use (
            ...
            return Tracer::inSpan(['name' => 'grpc_execute_streaming_sql_in_call()'], [$this->connection, 'executeStreamingSql'], [[
                'sql' => $sql,
                'session' => $session->name(),
                'database' => $this->getDatabaseNameFromSession($session)
            ] + $options]);
        ...
        $s1 = hrtime(true);
        $result = null;
        Tracer::inSpan(['name' => 'create_result_obj'], function () use(&$result, $session, $call, $context) {
            $result = new Result($this, $session, $call, $context, $this->mapper);
        });

        BaseStub::$logs[] = ['create_result', 'Inside Op execute, to create Result obj.', hrtime(true) - $s1];
        BaseStub::$logs[] = ['op_execute', '', hrtime(true) - $start];
        return $result;
    }
