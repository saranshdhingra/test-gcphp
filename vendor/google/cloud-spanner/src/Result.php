...
class Result implements \IteratorAggregate
{
    ...
    public function __construct(
        ...
        Tracer::inSpan(['name' => 'create_generator'], function () {$this->createGenerator();});
    }
    ...
    private function createGenerator()
    {
        ...
        $valid = Tracer::inSpan(['name' => 'result_backoff_execute'], [$backoff, 'execute'], [function () use ($call, &$generator) {
            $start = hrtime(true);
            $generator = Tracer::inSpan(['name' => 'call_inside_backoff_execute'], function () use($call){return $call();});
            BaseStub::$logs[] = ['result_call', 'Time taken to run $call inside backoff->execute', hrtime(true) - $start];
            $s1 = hrtime(true);
            $valid = Tracer::inSpan(['name' => 'generator_valid_inside_backoff_execute'], [$generator, 'valid']);
            BaseStub::$logs[] = ['result_gen_valid', 'Time taken to return generator->valid inside backoff->execute', hrtime(true) - $s1];
            BaseStub::$logs[] = ['result_backoff_execute', 'Time taken inside backoff->execute', hrtime(true) - $start];
            return $valid;
        }]);
        if ($valid) {
            // Multiple calls to the current method yields the same value.
            $start = hrtime(true);
            $result = Tracer::inSpan(['name' => 'result_generator_current'], [$generator, 'current']);
            BaseStub::$logs[] = ['result_gen_current', 'Time taken to return generator->current.', hrtime(true) - $start];
            $start = hrtime(true);
            Tracer::inSpan(['name' => 'setSnapshotOrTransaction'], function () use($result){
                $this->setSnapshotOrTransaction($result);
            });
            BaseStub::$logs[] = ['result_set_snapshot', '', hrtime(true) - $start];
