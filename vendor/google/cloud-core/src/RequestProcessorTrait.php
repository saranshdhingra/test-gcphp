...
trait RequestProcessorTrait
{
    ...
    private function handleResponse($response)
    {
        ...
        if ($response instanceof ServerStream) {
            return Tracer::inSpan(['name' => 'handleStream'],function() use($response){
    ...
    private function handleStream(ServerStream $response)
    {
        try {
            $start = hrtime(true);
            $all = Tracer::inSpan(['name' => 'response->readAll()'], function() use($response){
                return iterator_to_array($response->readAll());
            });
            BaseStub::$logs[] = ['handleStream_readAll', '', hrtime(true) - $start];
            foreach ($all as $count => $result) {
                $start = hrtime(true);
                $res = Tracer::inSpan(['name' => 'serializer_encodeMessage'], [$this->serializer, 'encodeMessage'], [$result]);
                BaseStub::$logs[] = ['handleStream_encodeMsg'.($count+1), 'Time taken to return encodeMessage #' . ($count+1), hrtime(true) - $start];
                yield $res;
            }
        } catch (\Exception $ex) {
